            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):#det代表該圖片有幾個物件框
                    if save_txt:  # Write to file
                        ...
                        equip_dictionary = {}
                        #time_now = datetime.now()
                        equip_dictionary["original_id"] = data_id
                        equip_dictionary['time'] = update_time
                        split_path = str(p).split("vue-V8\\vue-V8\\")
                        #print("===", split_path)
                        imagePath = split_path[1] if len(split_path) > 1 else ""
                        equip_dictionary['image'] = {"visible_light" : imagePath, "thermal": imagePath}
                        
                        equip_dictionary["category"] = which_equip(str(int(cls.cpu().item())))
                        #機率抱錯，對上下限存疑
                        xmin = int(xyxy[0].cpu().item())
                        ymin = int(xyxy[1].cpu().item())
                        xmax = int(xyxy[2].cpu().item())
                        ymax = int(xyxy[3].cpu().item())

                        equip_dictionary["coordinate"] = {"xmin" : xmin, "ymin" : ymin, "xmax" : xmax, "ymax" : ymax}
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default = yolo_detect_path, help='file/dir/URL/glob/screen/0(webcam)')
    ...
    print_args(vars(opt))
    return opt
def main(opt, tempList, data_ID, updateTime):
    check_requirements(exclude=('tensorboard', 'thop'))
    run(**vars(opt), tempList = tempList, data_id = data_ID, update_time = updateTime)
...
class worker(threading.Thread):
    ...
    def run(self):
        while self.queue.qsize() > 0:
            ...
            self.yolo_detect_path = self.fir.save_images(today+"-"+"{:03d}".format(day_counter))#原圖轉成可視影像和熱影像(無其他字擋住
            #LOCK??
...
if __name__ == '__main__':
    ...
    #collection2.insert_one({"number": 0})
    #資料夾名稱變數
    day_counter = 0
    today_0 = ""
    ...
    #檢查是否有連線到(好像不用try except)
        try:
            start_num = collection2.find_one()
            if start_num == None:
                print("===MongoDB can't find detect number===")
                time.sleep(1)
                continue
        except:
            continue
    #若number=1，讀取path_list資料夾位置裡的影像
        if start_num["number"] == 1:
            ...
            try:
                ...
                print("===MongoDB can't find unprocessed images===")
                time.sleep(1)
        
                continue

            detect_path = "vue-V8\\vue-V8\\turbo_img\\"
            path_list = sorted(image_data["image"])
            dir_name = image_data["time"]
            today = dir_name.split("-")[0]

            #生成turbo資料夾名稱
            if today_0 != today:
                day_counter = 1
                while os.path.exists(os.path.join(detect_path, today+"-"+"{:03d}".format(day_counter))):
                    day_counter += 1
            else:
                day_counter += 1
                while os.path.exists(os.path.join(detect_path, today+"-"+"{:03d}".format(day_counter))):
            ...
            #path_base = "C:\\Users\\ki\\Desktop\\python\\vue-V8\\vue-V8\\" ====絕對路徑
            path_base = "vue-V8\\vue-V8\\"
            img_names = []
            TempNp = np.zeros(len(path_list)).tolist()
            print(path_list)
            img_gueue = queue.Queue()
            #原本影像路徑的list
            for i in range(len(path_list)):
                img_names.append(path_base + path_list[i])#舊方式: 加入list
                img_gueue.put([str(path_base + path_list[i]), i])#將待處理路徑和其編號(length = 2的list)加入佇列
            ...
            workers = []
            t1 = time.time()
            print("generating turbo img...")
            for i in range(10):
                fir.append(flir_image_extractor.FlirImageExtractor())
            ...
            #     try:
            #         counter += 1
            #         print("==image{:03d}==".format(counter))
            #         start = time.time()
            #         fir.process_image(str(img_name))
            #         end1 = time.time()
            #         yolo_detect_path.append(fir.save_images(today+"-"+"{:03d}".format(day_counter)))#原圖轉成可視影像和熱影像(無其他字擋住
            #         end2 = time.time()
            #         TempNp.append(fir.get_thermal_np())
            #         end3 = time.time()
            #         print("process time",format(end1-start))
            #         print("save time", format(end2-end1))
            #         print("get np", format(end3-end2))
            #     except:
            #         print(img_name, " --is not IR images--")

            #該data的"processed" : False => True
            query = {"_id" : image_data["_id"]}
            update = {"$set": {"processed": True}}
            collection3.update_one(query, update)
                
            print("len of list", len(TempNp))

            #若有turbo照片，進yolo辨識
            if yolo_detect_path != "":
                print(yolo_detect_path)
                opt = parse_opt()
                main(opt, tempList = TempNp, data_ID = image_data["_id"], updateTime = dir_name)
            
            #number改回0
            query2 = {"_id" : start_num["_id"]}
            update2 = {"$set": {"number": 0}}
            collection2.update_one(query2, update2)

...
class FlirImageExtractor:
    ...
    def __init__(self, exiftool_path="temp_point\\yolov5\\read_thermal\\exiftool.exe", is_debug=False):
        ...
        self.thermal_suffix = ".png"
        self.default_distance = 1.0

        # valid for PNG thermal images           r"C:\Users\ki\Desktop\python\temp_point\yolov5\read_thermal\exiftool.exe"
    ...
    def process_image(self, flir_img_filename):
        ...
        #self.rgb_image_np = self.extract_embedded_image() #註解掉以加速辨識
    ...
    def orientation_change(self, img, orien):
        if orien == 1:
    ...
    def extract_embedded_image(self):
        ...
        ####
        test_img = Image.open(self.flir_img_filename)
        ori_num = self.exif_transpose(test_img)
        ###
        visual_img = Image.open(visual_img_stream)   
        visual_img = self.orientation_change(visual_img, ori_num)
    ...
    def extract_thermal_image(self):
        ...
        thermal_img = self.orientation_change(thermal_img, ori_num)
        thermal_np = np.array(thermal_img)
        #print("npnp", np.shape(thermal_np))
    ...
    def save_images(self, dirName):
        """
        Save the extracted images
        :return:
        """
        #rgb_np = self.get_rgb_np()                   #註解掉的
        #thermal_np = self.extract_thermal_image()    #註解掉的
        thermal_np = self.thermal_image_np

        #img_visual = Image.fromarray(rgb_np)         #註解掉的 為了加速辨識
        ...
        #save_path = "C:\\Users\\ki\\Desktop\\python\\vue-V8\\vue-V8\\turbo_img\\"
        save_path = "vue-V8\\vue-V8\\turbo_img\\"

        if os.path.isdir(save_path+str(dirName)) == False:
            os.mkdir(save_path+str(dirName))

        _, imgName = os.path.split(self.flir_img_filename)
        imgName = imgName.split(".")[0]
        thermal_filename = save_path + str(dirName) +  "\\" + imgName + self.thermal_suffix

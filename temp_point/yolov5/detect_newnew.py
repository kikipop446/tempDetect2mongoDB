...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            all_image_dic[img_number]["src"] = p
            all_image_dic[img_number]["result"] = {}
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        equipment_number = "{:02d}".format(equip_number)
                        all_image_dic[img_number]["result"][equipment_number] = {}
                        all_image_dic[img_number]["result"][equipment_number]["Type"] = which_equip(str(int(cls.cpu().item())))
                        ...
                        all_image_dic[img_number]["result"][equipment_number]["address"] = [xmin, ymin, xmax, ymax]
                        
                        thermal_np = tempList[counter]
                        #print(np.shape(tempList[counter][ymin:ymax, xmin:xmax]))
                        thermalCut = thermal_np[ymin:ymax, xmin:xmax]
                        #print(np.shape(thermalCut))
                        all_image_dic[img_number]["result"][equipment_number]["state"] = condition(equip = str(int(cls.cpu().item())), temp_cut = thermalCut)
                        tempMax = round(np.max(thermalCut), 4)
                        tempMin = round(np.min(thermalCut), 4)
                        tempAvg = round(np.mean(thermalCut), 4)
                        all_image_dic[img_number]["result"][equipment_number]["temp"] = {"temp_max" : tempMax, "temp_min" : tempMin, "temp_avg" : tempAvg}

                        # thermal_np = tempList[counter]
                        # thermal_np[ymin:ymax, xmin:xmax] = 0
                        # thermal_normalized = (thermal_np - np.amin(thermal_np)) / (np.amax(thermal_np) - np.amin(thermal_np))
                        # a = np.uint8(cm.turbo(thermal_normalized) * 255)
...
def main(opt, tempList):
    ...
    all_dic = run(**vars(opt), tempList = tempList)
    return all_dic
